...
def setup_logging(output_dir: str) -> None:
    ...
    log_path = os.path.join(output_dir, "train_binary_xgb.log")
...
def load_config(path: str | None) -> tuple[int, float, dict, dict, dict, str, str]:
    ...
    output_dir = "artifacts/xgboost/binary"
...
def train_model(
...
) -> tuple[xgb.XGBClassifier, dict, pd.DataFrame, pd.Series, pd.Series]:
    """训练 XGBoost 二分类模型"""
    log_subsection("XGBoost 二分类模型训练 (Normal vs Abnormal)")
    ...
    for k, v in merged_params.items():
        if k not in ["objective", "random_state", "n_jobs", "eval_metric"]:
...
def _save_binary_visualizations(
    ...
    def safe_confusion_percent(y_pred: np.ndarray) -> np.ndarray:
    ...
    def compute_threshold_curve(thresholds: np.ndarray) -> dict[str, np.ndarray]:
        precision_list = []
        recall_list = []
        specificity_list = []
        f1_list = []
        ...
        return {
            "precision": np.asarray(precision_list, dtype=float),
            "recall": np.asarray(recall_list, dtype=float),
            "specificity": np.asarray(specificity_list, dtype=float),
            "f1": np.asarray(f1_list, dtype=float),
            "accuracy": np.asarray(accuracy_list, dtype=float),
        }

    y_pred_opt = (y_proba >= float(optimal_threshold)).astype(int)

    cm_pct = safe_confusion_percent(y_pred_opt)
    for lang in ("zh", "en"):
    ...
    curve = compute_threshold_curve(thresholds)
    for lang in ("zh", "en"):
        fig, ax = plt.subplots(figsize=(6.2, 4.2))
        ax.plot(thresholds, curve["accuracy"], label=("准确率" if lang == "zh" else "Accuracy"))
        ax.plot(thresholds, curve["precision"], label=("精确率" if lang == "zh" else "Precision"))
        ax.plot(thresholds, curve["recall"], label=("召回率" if lang == "zh" else "Recall"))
        ax.plot(thresholds, curve["specificity"], label=("特异度" if lang == "zh" else "Specificity"))
        ax.plot(thresholds, curve["f1"], label=("F1" if lang == "zh" else "F1"))
        ax.axvline(float(optimal_threshold), color="k", linestyle="--", linewidth=1)
    ...
    summary["threshold_0.5"] = 0.5
    summary["threshold_optimal"] = float(optimal_threshold)
    summary["accuracy_0.5"] = float(accuracy_score(y_true, y_pred_05))
    ...
    for lang in ("zh", "en"):
        ...
        rows = []
        for k in keys:
            rows.append([k, f"{summary.get(k, float('nan')):.6f}" if isinstance(summary.get(k), float) else str(summary.get(k))])
...
def save_metrics_and_plots(
    ...
    metrics_path = os.path.join(output_dir, "metrics_binary_xgb.json")
    ...
    if cm.size > 0:
        ...
        ax.set_xlabel("预测类别")
        ax.set_ylabel("真实类别")
        plt.title("XGBoost 二分类混淆矩阵 (%)")
        plt.tight_layout()
        fig_path = os.path.join(output_dir, "confusion_binary_xgb.png")
        plt.savefig(fig_path, dpi=150)
        plt.close(fig)
    ...
    if fpr and tpr:
        ...
        ax.set_xlabel("假阳性率")
        ax.set_ylabel("真阳性率")
        ax.set_title("XGBoost ROC 曲线")
        ax.legend()
        plt.tight_layout()
        fig_path = os.path.join(output_dir, "roc_binary_xgb.png")
        plt.savefig(fig_path, dpi=150)
        plt.close(fig)
    ...
    if y_true is not None and y_proba is not None:
        ...
        _save_binary_visualizations(
            ...
            model_name_zh="XGBoost",
            model_name_en="XGBoost",
            optimal_threshold=thr,
            evals_result=evals_result,
        )


def save_model(model: xgb.XGBClassifier, feature_names: list[str], output_dir: str, seed: int, sampling_config: dict) -> str:
    ...
    path = os.path.join(output_dir, "binary_model_xgb.pkl")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="XGBoost 二分类模型训练")
