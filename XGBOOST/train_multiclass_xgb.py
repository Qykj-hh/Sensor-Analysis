...
def setup_logging(output_dir: str) -> None:
    ...
    log_path = os.path.join(output_dir, "train_multiclass_xgb.log")
...
def load_config(path: str | None) -> tuple[int, float, dict, dict, str, str]:
    ...
    output_dir = "artifacts/xgboost/multiclass"
...
def train_model(
...
) -> tuple[xgb.XGBClassifier, dict, pd.DataFrame, pd.Series, np.ndarray, pd.Series]:
    """训练 XGBoost 多分类模型"""
    log_subsection("XGBoost 多分类模型训练 (异常场景识别)")
    ...
    for k, v in merged_params.items():
        if k not in ["objective", "random_state", "n_jobs", "num_class", "eval_metric"]:
...
def save_metrics_and_plots(metrics: dict, label_mapping: dict[str, int], output_dir: str) -> None:
    ...
    metrics_path = os.path.join(output_dir, "metrics_multiclass_xgb.json")
    ...
    if cm.size > 0:
        ...
        plt.title("XGBoost 多分类混淆矩阵 (%)")
        plt.xlabel("预测类别")
        plt.ylabel("真实类别")
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        fig_path = os.path.join(output_dir, "confusion_multiclass_xgb.png")
        plt.savefig(fig_path, dpi=150)
        plt.close(fig)
        logging.info("  混淆矩阵: %s", fig_path)


def save_model(model: xgb.XGBClassifier, feature_names: list[str], label_mapping: dict[str, int],
    ...
    path = os.path.join(output_dir, "multiclass_model_xgb.pkl")
...
def run_training(data_dir: str, output_dir: str, test_size: float, seed: int,
    ...
    save_model(model, feature_names, label_mapping, models_dir, seed, sampling_config)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="XGBoost 多分类模型训练")
