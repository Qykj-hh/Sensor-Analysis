...
def setup_logging(output_dir: str) -> None:
    ...
    log_path = os.path.join(output_dir, "train_multiclass_rf.log")
...
def load_config(path: str | None) -> tuple[int, float, dict, dict, str, str]:
    ...
    output_dir = "artifacts/random_forest/multiclass"
...
) -> tuple[RandomForestClassifier, dict, pd.DataFrame, pd.Series, np.ndarray, pd.Series]:
    """训练随机森林多分类模型"""
    log_subsection("随机森林多分类模型训练 (异常场景识别)")
...
def save_metrics_and_plots(metrics: dict, label_mapping: dict[str, int], output_dir: str) -> None:
    ...
    metrics_path = os.path.join(output_dir, "metrics_multiclass_rf.json")
    ...
    if cm.size > 0:
        ...
        plt.title("随机森林多分类混淆矩阵 (%)")
        plt.xlabel("预测类别")
        plt.ylabel("真实类别")
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        fig_path = os.path.join(output_dir, "confusion_multiclass_rf.png")
        plt.savefig(fig_path, dpi=150)
        plt.close(fig)
        logging.info("  混淆矩阵: %s", fig_path)


def save_model(model: RandomForestClassifier, feature_names: list[str], label_mapping: dict[str, int],
    ...
    path = os.path.join(output_dir, "multiclass_model_rf.pkl")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="随机森林多分类模型训练")
