...
def setup_logging(output_dir: str) -> None:
    ...
    log_path = os.path.join(output_dir, "train_binary_svm.log")
...
def load_config(path: str | None) -> tuple[int, float, dict, dict, dict, str, str]:
    ...
    training_config = {
        ...
        "use_scaling": True,  # SVM通常需要特征缩放
    }
    data_dir = "test-datasets"
    output_dir = "artifacts/svm/binary"
...
def train_model(
...
) -> tuple[SVC, StandardScaler, dict, pd.DataFrame, pd.Series, pd.Series]:
    """训练SVM二分类模型"""
    log_subsection("SVM二分类模型训练 (Normal vs Abnormal)")
    ...
    use_scaling = training_config.get("use_scaling", True)
    
    logging.info("  平衡策略: %s", balance_strategy)
    logging.info("  特征缩放: %s", use_scaling)
    ...
    # 由于SVM没有内置特征重要性，我们可以通过特征的系数来近似
    log_subsection("Top 20 特征重要性（基于系数绝对值）")
    feature_names = X.columns.tolist()
    if hasattr(model, 'coef_') and model.coef_ is not None:
        # 对于二分类问题，coef_ 是一个形状为 (1, n_features) 的数组
        coef_abs = np.abs(model.coef_[0])
        feat_imp = sorted(zip(feature_names, coef_abs), key=lambda x: x[1], reverse=True)
    ...
    return model, scaler, metrics, X_valid, y_valid, paths_valid
...
def save_metrics_and_plots(metrics: dict, output_dir: str) -> None:
    ...
    metrics_path = os.path.join(output_dir, "metrics_binary_svm.json")
    ...
    if cm.size > 0:
        ...
        ax.set_xlabel("预测类别")
        ax.set_ylabel("真实类别")
        plt.title("SVM二分类混淆矩阵 (%)")
        plt.tight_layout()
        fig_path = os.path.join(output_dir, "confusion_binary_svm.png")
    ...
    if fpr and tpr:
        ...
        ax.legend()
        plt.tight_layout()
        fig_path = os.path.join(output_dir, "roc_binary_svm.png")
        plt.savefig(fig_path, dpi=150)
        plt.close(fig)
...
def save_model(model: SVC, scaler: StandardScaler, feature_names: list[str], output_dir: str, seed: int, sampling_config: dict) -> str:
    ...
    path = os.path.join(output_dir, "binary_model_svm.pkl")
...
def run_training(data_dir: str, output_dir: str, test_size: float, seed: int,
    ...
    model, scaler, metrics, X_valid, y_valid, paths_valid = train_model(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SVM二分类模型训练")
